...
$formData = $_POST['subregion'];


// parse the string
parse_str($formData, $subregion);


// dear future dev , kindly keep branch before subregion as subregion is reinitialized;

$branch = $subregion['branch'];

$subregion = $subregion['targetSR'];
...
if ($uploadOk == 0) {
    ...
    if (move_uploaded_file($_FILES['doc']['tmp_name'], $absolute_dir)) {
        ...
        echo json_encode(readExcel($con, $absolute_dir, $dir, $subregion));

        $checkId = uniqid();

        // insert check 
        $insertCheck = "INSERT INTO branchOrderChecks(checkId,branch) VALUES('$checkId','$branch')";
...
function readExcel($con, $absolute_dir, $dir, $subregion)
{
    ...
    $link = "<a href='../../server/p-o-c-l-s-r.php?d=" . $dir . "&&r=" . $subregion . "'><button class='ui green basic button'><i class='print icon'></i>Print</button></a>";
    ...
    foreach ($cleanArray as $product) {
        ...
        if ($subregion == 'All') {
            ...
            $sel = "SELECT productId, productName,inStock,inStockPieces,branch,packsize FROM products WHERE productName='$productName' AND subregion='$subregion'";
        }
